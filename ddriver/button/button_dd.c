...
static int button_open(struct inode* inode, struct file *file)
{
	printk(KERN_INFO "[BUTTON] Device File Opened\n");
	return 0;
}
...
{	
   	printk(KERN_INFO "[BUTTON] Write function\n");
   	return 0;
}
...
{	
	if(cmd == IOCTL_PID)
	{
		...
		{
			printk(KERN_INFO "[BUTTON] copy_from_user failed\n"); 
			return -1;
		}
		// printk(KERN_INFO "[PIR] PID-> %d\n", pid);
		printk(KERN_INFO "[BUTTON] Requested by PID %d\n", pid);
	}
	else
	{
		printk(KERN_INFO "[BUTTON] ioctl failed\n");	
	}
	return 0;
}
...
static int __init button_driver_init(void) 
{
	...
	{
		printk(KERN_INFO "[BUTTON] Cannot allocate major number\n");
	    return -1;
	}
	...
	{
		printk(KERN_INFO "[BUTTON] Cannot add the device to the system\n");
	...
	{
		printk(KERN_INFO "[BUTTON] Cannot create the struct class\n");
		unregister_chrdev_region(dev,1);
	}
	...
	{
		printk(KERN_INFO "[BUTTON] Cannot create the Device\n");
	...
	{
		printk(KERN_INFO "[BUTTON] Cannot register IRQ\n");
...
static void __exit button_driver_exit(void) 
{
	...
	cdev_del(&c_dev);
	unregister_chrdev_region(dev, 1);
	printk(KERN_INFO "[BUTTON] Device Driver Remove\n");
}
